...
for i in range(36,180):
    urlString = "https://nvd.nist.gov/vuln/search/results?form_type=Advanced&results_type=overview&search_type=all&cwe_id=CWE-89&startIndex=" + str(i) + ""
    body = urllib2.urlopen(urlString)
    soup = BeautifulSoup(body)
    parse_tr = soup.find_all('tr data-testid')[0]

    columns = []

    for row in parse_tr.find_all('td', {'style': 'text-align:left'}):
...
workbook.save('contractName_jeonraNam.xls')
