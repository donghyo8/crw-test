...
for i in range(0, len(strlist)):
    urlString = "http://www.provin.gangwon.kr/gw/portal/sub05_05_04?firstIndex=" + strlist[i]
    body = urllib2.urlopen(urlString)
    soup = BeautifulSoup(body)
    parse_tr = soup.find_all('tbody')[0]

    for row in parse_tr.find_all('tr', {'align': 'center'}):
        columns = row('td')[5]
        print("Page Numer : " + strlist[i])
...
workbook.save('contractName_kangwon.xls')
