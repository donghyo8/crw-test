...
def bidnum_writer(name):
    a = []
    f = open(name + ".txt", 'w')
    for i in range(350, 1971):
        data = str(i) + '\n'
        f.write(data)
    f.close()

bidnum_list2 = bidnum_writer('bidnum')
...
for i in range(0, len(strlist)):
    urlString ="https://www.gg.go.kr/special-contract/"+ strlist[i]
    body = urllib2.urlopen(urlString)
    soup = BeautifulSoup(body)
    tb_inner_count = 0
    parse_tr = soup.find_all('tbody')[0]

    columns = []
    for row in parse_tr.find_all('td', {'class': 'brdTitle'}):
        columns = row
...
workbook.save('contractName_kyungki.xls')
